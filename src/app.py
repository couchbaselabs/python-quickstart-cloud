...
class CouchbaseClient(object):
    ...
    def connect(self, **kwargs):
        ...
        try:
            ...
            createIndexProfile = (
                f"CREATE PRIMARY INDEX default_profile_index ON {self.bucket_name}"
            )
...
class Profiles(Resource):
    ...
    def get(self):
        try:
            ...
            query = f"SELECT p.* FROM  {db_info['bucket']} p WHERE lower(p.firstName) LIKE $search_str OR lower(p.lastName) LIKE $search_str LIMIT $limit OFFSET $offset;"
